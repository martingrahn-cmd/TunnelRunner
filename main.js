...
// ── Sound toggle ──
// All audio comes from the ElevenLabs-generated assets via audio.js (see
// AUDIO.md). An earlier pass added a parallel WebAudio oscillator layer whose
// nearMiss/levelUp/coin/start beeps fired *alongside* those samples, so those
// events played twice. The synth is gone; this toggle now drives the real
// mixer's master mute, which is also what M does.
let sfxEnabled = !audio.muted;
...
function showLevelUp() {
  ...
  setTimeout(() => {
    ...
    // Build the new level's course. The level-up path clears the old obstacles
    // so the boost zone is empty, but nothing rebuilt them afterwards — every
    // level after the first was a completely empty tunnel. generateObstacles()
    // reseeds the layout rng from runSeed ^ level, so each level is a fresh
    // course and practice mode stays deterministic; generateCoins() must follow
    // it, since coins are placed relative to the gaps it just laid out.
    generateObstacles();
    generateCoins();
    // Seed the pass-detection state against where the ship actually is, or
    // everything behind it counts as freshly dodged the moment it spawns.
    obstacles.forEach(o => { o.lastTd = tDist(progress, o.t); });
...
// arrive.
//
// Uses the seeded layout rng, not Math.random, and is always called directly
// after generateObstacles() — so a given seed lays out the same coins as well
// as the same obstacles, which is what practice mode and the ghost line need.
function generateCoins() {
  ...
  for (let i = 0; i < obstacles.length; i++) {
    ...
    // Anchored to the *visual* gap edge (sliceAngle/2) and pulled back by a
    // fixed margin, rather than to a fraction of the gap: a fraction lands
    // deep in safe territory on wide patterns and only bites on narrow ones,
    // whereas measuring from the edge costs the same margin everywhere.
    //
    // The safe cone is deliberately wider than the visual gap now
    // (gapHalfCos uses sliceAngle/2 + FAIR_HIT_PAD), so a coin inside the
    // visual gap is always survivable — it just spends nearly all of the
    // forgiveness. That also puts the player at or near the near-miss band
    // (which opens at sliceAngle/2 + FAIR_HIT_PAD - FAIR_NEAR_MISS_INNER),
    // so a well-flown risk pickup tends to pay the streak bonus as well.
    if (rng() < 0.38) {
      const visualHalf = obs.sliceAngle / 2;
      const offset = Math.max(0, visualHalf - RISK_COIN_EDGE_MARGIN);
...
function collectCoin(coin) {
  ...
  // Keeps the bloom/shake juice from the visual rework, and scales it up for
  // risk coins so the expensive pickup reads as the bigger moment.
  const sparkCount = coin.risk ? 26 : 18;
  boostBloomTarget = Math.max(boostBloomTarget, coin.risk ? 0.7 : 0.55);
  camShake = Math.max(camShake, coin.risk ? 0.22 : 0.14);
  for (let i = 0; i < sparkCount; i++) {
    const geo = new THREE.OctahedronGeometry(0.12 + Math.random() * 0.22);
    const mat = new THREE.MeshBasicMaterial({
      color: Math.random() > 0.35 ? (coin.fxColor ?? 0xffdd44) : 0xffffff,
      transparent: true,
      opacity: 1,
      fog: false,
      blending: THREE.AdditiveBlending,
